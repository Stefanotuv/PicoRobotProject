...
def application_mode():
    ...
    def app_motor_move(request):
        ...
        if motor_control == "up":
            motor_car.move_forward(1)
        elif motor_control == "down":
            motor_car.move_backward(1)
        elif motor_control == "left":
            motor_car.move_left(.5)
        elif motor_control == "right":
            motor_car.move_right(.5)
        else:
            motor_car.move_stop()
            print("stop")
        return ""
...
# back_servo_camera_tilt = Servo(pin=3)
# back_servo_camera_pan = Servo(pin=2)
